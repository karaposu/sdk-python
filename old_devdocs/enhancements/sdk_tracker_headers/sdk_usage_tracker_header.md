...
**Why User-Agent doesn't include operation type:**

User-Agent is set **once** at session creation and remains **static** for all requests. It cannot change per-operation because:

1. **Session-level header** - `aiohttp.ClientSession` sets default headers at creation time
2. **Performance** - Creating a new session per operation would be expensive
3. **Simplicity** - Overriding headers per-request adds complexity

To track operation-specific data, the SDK uses a **separate mechanism** (`sdk_function` query parameter) instead of modifying User-Agent per request.

**Alternative approach (not used):**
```python
# Could override per request, but adds overhead
async def products(self, url):
    headers = {"User-Agent": f"brightdata-sdk/2.1.0 (op=products)"}
    await self.engine.post(..., headers=headers)
```

The current design splits concerns:
- **User-Agent** → Static identity (SDK name + version)
- **sdk_function** → Dynamic per-operation tracking
